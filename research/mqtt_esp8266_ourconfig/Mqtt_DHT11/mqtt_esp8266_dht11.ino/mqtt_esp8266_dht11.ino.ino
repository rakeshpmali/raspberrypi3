...
unsigned char g_apduTx[] = {7, 0x01, 0x02, 0x03, 0x04, 0x02,
...
void reconnect() {
  ...
  while (!client.connected()) {
    ...
    // Attempt to connect
    if (client.connect("ESP8266Button")) {
  ...
  if (now - lastMsg > 2000) {
    ...
    if (isnan(t)) {
      ...
      send_iso_case_4(g_apduTx, &rxLen, g_rxData);
      {
        unsigned int i;
        for(i = 0; i < rxLen; i++){
          Serial.print(g_rxData[i],HEX);
        }
        Serial.println("...end");
      }
      client.publish("outTopic", (const char*)g_rxData, rxLen);  
    }
  }
}
